...
/// must fail closed (AD-12): a host that has not yet bound the real read path renders a safe failure surface rather
/// than fabricating a "ready/healthy" Agent or an empty "no providers configured" success. The component
/// tests substitute the gateways, so this contract is otherwise unexercised.
/// </summary>
public sealed class DeferredGatewayTests
{
    [Fact]
    public async Task DeferredAgentSetupGateway_status_is_unavailable_with_no_agent()
    {
    ...
    [Fact]
    public async Task DeferredAgentSetupGateway_configuration_is_unavailable_with_no_agent()
    {
        DeferredAgentSetupGateway gateway = new();
    ...
    [Fact]
    public async Task DeferredAgentSetupGateway_setup_is_unavailable_with_no_setup()
    {
        DeferredAgentSetupGateway gateway = new();

        AgentSetupResult result = await gateway.GetSetupAsync(expectedConfigurationVersion: 7, CancellationToken.None);
