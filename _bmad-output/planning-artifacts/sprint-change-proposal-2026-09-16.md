...
- Agents `Directory.Packages.props` is import-only and carries no package-specific version property, `PackageVersion`, or fallback list. `BuildContractConformanceTests` covers root, sibling, parent-references, and missing-catalog behavior; the shared consumer-authority validator covers the real repository plus local-property, local-item, and `VersionOverride` authority violations.
...
- Builds `origin/main` contains catalog/audit target `000abf867abc3a99cfa74d39b6e73af05c78a602`, and Agents commit `01ead38` published on `origin/main` records that exact gitlink. The publication condition is satisfied, but `ARCH-A-15` remains open pending exact Security + Builds Maintainer approval of `OD-DAPR-SECURITY-1`: Story 5.1 compatibility evidence covers the consumed Client/ASP.NET/Actors graph only, while Workflow compatibility remains Story 6.1 work.
