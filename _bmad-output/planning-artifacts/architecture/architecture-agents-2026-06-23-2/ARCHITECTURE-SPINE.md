...
**Second-update trigger calculation:** The blocked-call and unavailable-call shares count distinct `(Party, Conversation, reason)` tuples over distinct `(Party, Conversation)` attempts in the rolling seven-day window, excluding only rate-limit rejections at Platform-Operator or Release-Operator limits; rejections at Tenant-Agent-Administrator-lowered limits count. Provider-or-generation failure share and posting-failure rate retain the PRD formulas. The ordinary minimum sample is 5 distinct calling Parties and 50 Agent Calls; a tenant with fewer than 5 calling Parties uses every Party that called and 20 calls. A smaller sample is `InsufficientEvidence`, not a pass. Exceeding a threshold requires a Release Operator review within one business day; absence is `TriggerReviewOverdue` and blocks qualification. The switch moves only on the recorded review decision, except the Platform Operator's immediate confirmed SM-4 response. Gate and launch-health metrics remain separate calculations.
...
| Name | Version |
| --- | --- |
| .NET SDK | `10.0.301` with `rollForward: latestPatch` from root `global.json`; siblings are on `10.0.400` (deviation ARCH-A-4); the `10.0.3xx` feature band's last patch (`10.0.303`) shipped in the 2026-08-11 monthly bundle (`10.0.11`), and the 2026-09-08 bundle (`10.0.12`) shipped with no `10.0.3xx` member, so the band is now out of active servicing — a build-supportability risk (no further security patches), not only test-stack debt — targeted no later than Story 5.6, sooner on any security advisory against `10.0.3xx` (see `dotnet/core` release notes; ARCH-A-4) |
...
| Hexalith.EventStore | parent gitlink `e302432c` at the spine `updated` date |
...
**Architecture Assumption Index:** `ARCH-A-INDEX-3`
...
| ARCH-A-4 | Root test-stack overrides (xunit.v3 3.2.2, NSubstitute 5.3.0, Microsoft.NET.Test.Sdk, xunit.runner.visualstudio, SDK 10.0.301) are aligned with the workspace catalog, including the Microsoft Testing Platform v2 `test.runner` entry in `global.json`, by Story 5.6; the SDK `10.0.3xx` feature band is additionally out of active servicing as of the 2026-09-08 bundle, escalating this row's target sooner than Story 5.6 on any security advisory against `10.0.3xx` | Stack | Architecture | Story 5.6 aligns them or records a permanent override reason, escalated sooner on a `10.0.3xx` security advisory | TBD — target milestone: Story 5.6, or immediately on advisory |
...
| ARCH-A-12 | The per-tenant `DigestKey` is never rotated while the tenant holds unerased protected content; rotation is permitted only after full-tenant erasure or an accepted-risk waiver | AD-22, AD-29 | Architecture + Security | Security confirms the rotation policy or a key-lifecycle procedure supersedes it | TBD — target milestone: before `RQ-1` evaluation |
