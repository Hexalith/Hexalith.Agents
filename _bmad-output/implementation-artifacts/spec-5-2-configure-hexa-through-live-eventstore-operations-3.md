...
### Review Findings (2026-09-23, re-review of `13857fd..0a77c73`)

- [x] [Review][Decision] A non-declared setup payload's permanent 400 rejection is unverified on the Agents gateway and absent from released EventStore — resolved 2026-09-23: option (a), with a mode-aware gateway assertion; converted to the patch below. Original detail: `NormalizeDeclaredPayload` throws `ArgumentException` for `null`, `[]`, or malformed payloads, and only EventStore submodule commit `42a7ff05` maps it to 400 / `retryable: false` / `correct_request`. No tag contains that commit: `v3.106.0` and `v3.107.0` still map every admission exception to 503 / `retry_later`, and the catalog and verifier floor stay at `3.106.0`. In package mode, which the Release and CI lanes use, a permanently invalid setup payload is reported as retryable. `A_non_agent_domain_fails_on_the_gateway_before_execution` asserts only `IsSuccessStatusCode == false`, so it passes on either mapping, and `null` / `[]` are exercised only at `registry.Resolve`. Adding a gateway assertion for 400 / `retryable: false` / `correct_request` fails in package mode until an EventStore release carries the catch and the Builds catalog moves, which this slice may not change. Options: (a) add the assertion scoped to source mode now and defer the floor raise to the EventStore release; (b) defer both until the release, recording it in deferred work; (c) publish EventStore first, then raise the catalog and floor and add the assertion in the dependency-maintenance work.
- [x] [Review][Patch] The gateway test does not pin the admission-rejection contract: assert 400 / `retryable: false` / `correct_request` against source-mode EventStore, and the published 503 / `retry_later` behavior against package mode, for a non-agent domain plus `null` and `[]` payloads [test/Hexalith.Agents.Server.Tests/AgentsEventStoreGatewayIntegrationTests.cs:513]
- [x] [Review][Patch] AC3's mixed Dapr app-id rejection is asserted only for `ActivateAgent` [test/Hexalith.Agents.Server.Tests/AgentsEventStoreGatewayIntegrationTests.cs:656]
- [x] [Review][Patch] The signed 64-bit wrap guard in `FromUnsignedOrdinal` is untested — `9223372036854775808` would decode as `WideSignedToleranceStatus.Minimum` if `<= long.MaxValue` were dropped [test/Hexalith.Agents.Contracts.Tests/AgentOperationContractsTests.cs:572]
- [x] [Review][Defer] Moved host-composition comments still say live command dispatch "stays deferred behind DeferredAgentCommandDispatcher" [src/Hexalith.Agents.Server/Composition/AgentDomainHostComposition.cs:72] — deferred: pre-existing. The comments moved verbatim from `Program.cs`, and they contradict the Story 5.2 `Agents:EventStore:BaseUrl` block in the same method.

**Rejected**

- A key completed before deployment conflicts on retry instead of replaying — `false` as a defect. The Always constraint requires that "a changed canonical form for an already-admitted key conflicts", and completed keys are admitted keys. Replaying instead would require renegotiating the frozen spec.
- A version-skewed gateway collapses new enum values or new fields into one intent — `false`. This was already adjudicated in BH-03, BH-04, and "a future optional property": declared-contract normalization is the approved design, and a later schema change needs its own identity decision.
- `catch (ArgumentException)` in `SubmitCommandHandler` also catches guard-clause failures that come from defects — `low`. The only other `ArgumentException` sources on the admission path are null/whitespace guard clauses that internal defects reach, and those deterministic failures would not succeed on retry either. A dedicated exception type adds public EventStore surface across repositories.
- Dependabot `gitsubmodule` bumps fail the literal Builds SHA check — already deferred ("Define the coupled update path for Dependabot proposals that advance the Builds gitlink").
- Public operation enums keep implicit ordinals, and provider and approver verdicts feed the activation intent — already deferred for the enums. The activation claim is `false`: `ActivationSemanticExtensionKeys` hashes only the administrator flag and the expected configuration version.
- CI does not run the Story 5.2 focused gates, and the job timeout rose to 90 minutes — already deferred. The timeout change names no harm.
- The status-reader timeout test throws synchronously, so the awaited path is untested — `false`. At `EventStoreAgentAdministrationOperations.cs:374-386`, both the invocation and the `await` sit inside the same `try`, so the two paths are handled identically.
- The composition test covers only the unconfigured path — `low`, previously rejected. The `BaseUrl` branch registers a gateway client and dispatcher, not admission adapters.
- The conflicting-registration check misses factory or type registrations — `false`. `AgentsTrustedCommandExtensionPolicy` is `internal` and is registered only as an instance by `AddAgentsEventStore`.
- A keyed `ITrustedCommandExtensionPolicy` descriptor makes `ImplementationInstance` throw — `low`. Nothing registers a keyed policy; EventStore resolves the policies unkeyed, and the fix is a new guard.
- Normalization catches only `JsonException`, and the domain executes raw bytes — `false`. Payload-driven failures for these fixed contract types surface as `JsonException`, and the domain binds with the same `EventStorePayloadSerialization.Options` (BH-03).
- The retired `AgentLifecycleStateAlreadySetRejection` is not `[Obsolete]` — `false`. No aggregate path emits it; it is kept for `AgentState.Apply` replay, and the finding names no harm.
- CI, release, Dependabot, and Builds changes violate the Never constraint — `false`. They come from spec -2 review commit `1d1aaff`, which falls inside the baseline range, and the File List attributes them to separate work.
- The padded or conflicting app-id guards violate the Never constraint — `false`, for the same reason. Previously rejected.
- Correlation tests, verifier regrouping, and doc rewrites are outside this spec's tasks — `false`. They come from spec -2 follow-up commits `db88b0f` and `1d1aaff` in the shared range.
- The removed `ShouldNotContain("AddAgentsEventStore(")` leaves `Program.cs` unguarded — `false`. `Hexalith.Agents.Server` does not reference `Hexalith.Agents.EventStore`, so `Program.cs` cannot call that registration.
- The in-flight conflict is proven only against the admission test double — `false`. The real actor's digest conflict is covered by EventStore's `AdmitAsync_LiveDifferentIntent_ReturnsConflictWithoutMutation` (BH2-07).
- `AgentSetupWriteStatus.Submitted = 0` and `AgentInspectionStatus.Success = 0` are frozen by the pin test — already deferred (DW-11, DW-13). Pinning current ordinals is exactly what the spec asks for.
