...
/// awaiting approval). The rest of the AD-5 lifecycle is reserved here additively so the proposal surface does not need a
/// contract change when those stories arrive.
/// </summary>
/// <remarks>
...
/// values in this story — <c>Regenerated</c> (Story 3.4), <c>Approved</c>/<c>Rejected</c>/<c>Abandoned</c>/<c>Expired</c>
/// (Stories 3.5/3.6), and <c>PostingPending</c>/<c>Posted</c>/<c>PostingFailed</c> (Story 3.5) are owned by Stories
/// 3.4–3.6 and must NOT be added here (mirroring how <see cref="AgentGenerationKind"/> reserves its deferred kind additively).
/// </remarks>
