...
/// <summary>
/// Resolves a persisted provider-catalog event type name to its public contract type. Only types under the
/// catalog event namespaces are resolvable, so a stray or hostile type name can never be materialized.
/// </summary>
public static class ProviderCatalogEventTypeResolver
{
    private const string _eventNamespace = "Hexalith.Agents.Contracts.ProviderCatalog.Events";
    ...
    private static FrozenDictionary<string, Type> Build()
    {
        ...
        Assembly contracts = typeof(ProviderModelEntryCreated).Assembly;

        foreach (Type type in contracts.GetExportedTypes().Where(IsCatalogEventType))
        {
            map[type.Name] = type;
